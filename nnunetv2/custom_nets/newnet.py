...
class EncoderStage(nn.Module):
    def __init__(
        ...
        for i in range(int(n_blocks)):
            stride = as_3tuple(first_stride) if i == 0 else (1, 1, 1)
            blocks.append(
                ResidualSEBlock3D(
...
class UNetResSE3D(nn.Module):
    ...
    def forward(
        ...
        seg_logits = seg_outputs if self.deep_supervision else seg_outputs[0]
...
class UNetResSE3DWithQueryDecoder(nn.Module):
    ...
    def inference_from_queries(
        ...
        mask_quality = mask_quality.float()
        ...
        for b in range(prob_np.shape[0]):
            centroids.append(
                connected_components_centroids_3d(prob_np[b] > float(mask_thresh))
            )
...
__all__ = [
    "UNetResSE3D",
    "UNetResSE3DWithQueryDecoder",
    "QueryInferenceResult",
    "connected_components_centroids_3d",
]