...
def _kernel_and_padding_3d(
    ...
    We precompute padding so ResidualSEBlock3D does not attempt `kernel_size // 2`
    when kernel_size is a tuple.
    """
...
def _stride_3d(stride: Union[int, Sequence[int]]) -> Union[int, Tuple[int, int, int]]:
    ...
    # Avoid stride=(1,1,1) because ResidualSEBlock3D uses (stride != 1) to decide projection.
    if s3[0] == s3[1] == s3[2] == 1:
        return 1
...
class _IdentityNorm(nn.Module):
    """A norm placeholder that accepts (num_features, **kwargs) like InstanceNorm/BatchNorm."""
        ...
        for b in range(n_blocks):
            ...
            blocks.append(
                ResidualSEBlock3D(
                    ...
                    out_ch=out_ch,
                    stride=stride_arg,      # int or tuple
                    kernel_size=k_arg,      # int or tuple
                    padding=p_arg,          # int or tuple
                    bias=bias,
                    conv_op=conv_op,
                    norm_op=norm_op_cls,
                    norm_kwargs={} if norm_op_kwargs is None else dict(norm_op_kwargs),
                    nonlin=nonlin_factory,  # factory returning nn.Module
...
class UNetResSE3D(nn.Module):
    """
    nnU-Net v2 plans-compatible 3D U-Net that uses ResidualSEBlock3D per conv stack.

    Signature intentionally matches (superset of) PlainConvUNet kwargs.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # kernel sizes per stage (nnU-Net stores them per stage, often as [3,3,3])
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        seg_outputs = list(reversed(seg_outputs))  # full-res first
