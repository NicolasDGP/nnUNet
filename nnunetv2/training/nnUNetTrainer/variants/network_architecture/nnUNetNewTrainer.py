...
class nnUNetTrainerResSEWithQueryDecoder(nnUNetTrainer):
    ...
    The important fix for your current crash:
    every train_step return path now returns the exact same dictionary keys.
    """
    ...
    def _all_tensors_finite(obj: Any) -> bool:
        ...
        if isinstance(obj, dict):
            return all(
                nnUNetTrainerResSEWithQueryDecoder._all_tensors_finite(v)
                for v in obj.values()
            )
